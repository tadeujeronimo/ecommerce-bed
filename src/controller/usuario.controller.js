...
const addUserFavShoeController = async (req, res) => {
  try {
    res
      .status(201)
      .send(await userService.addUserFavShoeService(req.params.id, req.body));
  } catch (err) {
    console.log(`erro: ${err.message}`);
    return res
      .status(500)
...
const removeUserFavShoeController = async (req, res) => {
  try {
    res
      .status(201)
      .send(
        await userService.removeUserFavShoeService(req.params.id, req.body)
      );
  } catch (err) {
    console.log(`erro: ${err.message}`);
    return res
...
module.exports = {
  ...
  addUserFavShoeController,
  removeUserFavShoeController,
};
